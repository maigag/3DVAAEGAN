...
Let’s define the inputs to our model as :
...
Let’s define the components of our network as :

  - \(E\) : the encoder

  - \(ED\) : the encoder discriminator
...
Then we can define the following functions :
  ...
  - \(E(y) = z\textunderscore\) : as the encoding of the datacost block

  - \(ED(z\textunderscore)\) or \(ED(z)\) : the output of the encoder
    discriminator given either the encoded block or the random noise

  - \(G(z\textunderscore)\) or \(G(z)\) : : the output of the generator
    given either the encoded block or the random noise

  - \(D(G(z\textunderscore))\) or \(D(G(z))\) : the output of the
    discriminator given either the reconstructed block or the generated
    block

Furthermore, in the same way as in Yu Nishimura’s work, we define the
reconstruction loss in the following way
:

\(L_{rec} = \sum_{n=1}^{\#Classes} w_{n}(-\gamma x\log(G(z\textunderscore)) - (1 - \gamma)(1-x)log(1 - G(z\textunderscore))\)  
  
where \(w_{n}\) weights the importance of certain classes of objects,
according to their occupancy in the current batch. \(\gamma\), instead,
is a hyperparameter that weights the importance of false negative
against false positive.

Finally, we can define the losses we want to minimize:

  - The encoder discriminator loss :
    \(L_{ED} = -log(ED(z)) - log(1 - ED(z\textunderscore))\)

  - The encoder loss :
    \(L_{E} = -log(ED(z\textunderscore)) + \alpha_{rec} L_{rec}\)

  - The discriminator loss :
    \(L_{D} = -log(D(x)) - log(1 - D(G(z))) - log(1 - D(G(z\textunderscore)))\)

  - The generator loss :
    \(L_{G} = -log(D(G(z))) - log(D(G(z\textunderscore))) + \alpha_{rec} L_{rec}\)

\(\alpha_{rec}\) is a hyperparameter weighting the importance of the
reconstruction loss. In summary, the optimization translates into :

  - \(L_{ED}\) gets pushed toward zero if it correctly classifies the
    encoding w.r.t the distribution they come from

  - \(L_{E}\) gets minimized on one hand if the encoder discriminator
    misclassifies its encoding and on the other hand, if the
    reconstruction gets better

  - \(L_{D}\) gets pushed toward zero if it correctly understand whether
    a block was generated by the generator or it comes from the dataset

  - \(L_{G}\) gets minimized on one hand, if the discriminator
    misclassifies both reconstructed and generated blocks, on the other
    hand if the reconstruction gets better compared to the groundtruth
    block
